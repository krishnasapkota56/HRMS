...
class Employee(models.Model):
    ...
    designation = models.CharField(
        max_length=50, choices=DesignationChoice.choices, default=DesignationChoice.Employee)
    date_of_joining = models.DateField(auto_now_add=True)
    date_of_birth = models.DateField()
    profile_image = models.ImageField(
        upload_to='profile_images/', max_length=100, null=True, blank=True)
    managers = models.ManyToManyField(
        'Employee',
        limit_choices_to={'designation': 'MGR'},
...
class Reimbursement(models.Model):
    ...
    reimbursement_type = models.CharField(
        max_length=50, choices=ReimbursementChoice.choices, default=ReimbursementChoice.TravelingA)
    title = models.CharField(max_length=150)
    description = models.TextField(max_length=300, blank=True, null=True)
    total_amount = models.DecimalField(max_digits=6, decimal_places=2)
    file_1 = models.FileField(
        upload_to='reimbursements/%Y/%m/%d', blank=True, null=True)
    ...
    approval = models.CharField(
        max_length=50, choices=ApprovalChoice.choices, default=ApprovalChoice.pending)
...
class Leave(models.Model):
    ...
    leave_type = models.CharField(
        max_length=50, choices=LeaveChoice.choices, default=LeaveChoice.FullDay)
    title = models.CharField(max_length=150)
    description = models.TextField(max_length=300, blank=True, null=True)
