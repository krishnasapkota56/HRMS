...
def reimbursementView(request):
    ...
    form = None
    table = EmployeeReimbursementTable(
        Reimbursement.objects.filter(user=request.user))
...
def approveLeaveView(request):
    ...
    notifications = request.user.notifications.unread()
    notification_count = notifications.count()
    employees_under_manager = Employee.objects.filter(
        managers=Employee.objects.get(user=request.user))
    ...
    table = ManagerLeaveApprovalTable(
        Leave.objects.filter(user__in=employee_under_manager_arr))
...
def approveReimbursementView(request):
    ...
    for employee in employees_under_manager:
        employee_under_manager_arr.append(employee.user)
    print(employee_under_manager_arr)
    table = ManagerReimbursementApprovalTable(
        Reimbursement.objects.filter(user__in=employee_under_manager_arr))
...
def approveMeetingView(request):
    ...
    for employee in employees_under_manager:
        employee_under_manager_arr.append(employee.user)
    print(employee_under_manager_arr)
    table = ManagerMeetingApprovalTable(
        Meeting.objects.filter(user__in=employee_under_manager_arr))
...
def dashboardView(request):
    ...
    data = Attendance.objects.filter(
        user=request.user, today_date=current_date).first()
    table = EmployeeAttendanceTable(
    ...
    reimbursements = Reimbursement.objects.filter(
        user=request.user, approval='accept')
    ...
    if request.method == 'GET':
        # request.query_params.get('click', None)
        click_state = request.GET.get('click', None)
        # Salary calculation
        reimbursements = Reimbursement.objects.filter(
            user=request.user, approval='accept')
        leaves = Leave.objects.filter(user=request.user, approval='accept')
        ...
        if click_state == 'start':
            if data:
                ...
                total_days_present = 30-total_leave
                total_salary = (
                    salary_per_day*total_days_present)+total_reimbursement
                ...
                Attendance.objects.create(
                    user=request.user, start_time=timezone.localtime(timezone.now()))
                return redirect('employee:dashboard')
            ...
            if data:
                if data.end_time:
                    ...
                    Attendance.objects.filter(pk=data.pk).update(
